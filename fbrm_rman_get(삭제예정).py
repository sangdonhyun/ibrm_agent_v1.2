...
class fbrm_ora():
    ...
    def rman(self):
        ...
        if len(cat_ps) > 4:
            cat_owner=cat_ps[0]
            print cat_owner
        os.popen('chomod 777 %s'%tar_file)
        user=self.cfg.get('rmancat','user')
        passwd=self.cfg.get('rmancat','passwd')
        dbname=self.cfg.get('rmancat','dbname')
    
        cmd="""su - oracle -c "export ORACLE_SID=catdb;sqlplus -s '%s/%s@%s '< %s" """%(user,passwd,dbname,tar_file)
        print cmd
    ...
    def set_sql_file(self,sid):
        src_file=os.path.join('sql','rman','fbrm_rman.sql')
        print src_file
        tar_file='/tmp/fbrm/rman_sql/fbrm_rman.sql'
        print src_file
