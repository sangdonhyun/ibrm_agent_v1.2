...
class fbrm_ora():
    ...
    def rman(self):
        ...
        tar_file=os.path.join('/tmp','fbrm','sql','fbrm_rman.sql')
        ...
        cmd="""su - oracle -c "export ORACLE_SID=catdb;sqlplus -s '%s/%s@%s '< /tmp/sql/fbrm_rman.sql" """%(user,passwd,dbname)
        print cmd
        os.popen(cmd)
    ...
    def get_vbackup_files(self,sid,owner,env_info):

        spool_file='/tmp/{HOSTNAME}_{DBNAME}_v_backup_files.tmp'
        spool_file=spool_file.format(HOSTNAME=self.hostname,DBNAME=sid)

        sql_file=os.path.join('sql_ora','v_backup_files.sql')
        ...
        lineset = lineset.replace('{DBNAME}',sid)
        print lineset
        tsql_file= os.path.join('/tmp','v_backup_files.sql')
        print tsql_file
        ...
        fname = '{HOSTNAME}_{DBNAME}_v_backup_file_{TIME}.tmp'
        ...
        with open(ora_tmp_file,'w') as fw:
            head_msg=self.getHeadMsg('{HOSTNAME}_{DBNAME}env_cnt.tmp')
            ...
            fw.write('###***ORACLE_SID***###\n')
            fw.write(sid + '\n')
            zfs_ip, zfs_name = self.get_zfs(sid)
            fw.write('###***ZFS_IP***###\n')
            fw.write(zfs_ip + '\n')
            fw.write('###***ZFS_NAME***###\n')
            fw.write(zfs_name + '\n')
        ...
        socketClient.SocketSender(FILENAME=ora_tmp_file, DIR='FBRM_ORA_BAKUP', ENDCHECK='NO').main()
        os.remove(ora_tmp_file)


    def get_vsession_longops(self,sid,owner,env_info):

        spool_file='/tmp/{HOSTNAME}_{DBNAME}_session_longops.tmp'
        spool_file=spool_file.format(HOSTNAME=self.hostname,DBNAME=sid)

        sql_file=os.path.join('sql_ora','session_longops.sql')
        with open(sql_file) as f:
            lineset=f.read()
        print lineset
        ...
        lineset = lineset.replace('{DBNAME}',sid)
        print lineset
        tsql_file= os.path.join('/tmp','session_longops.sql')
        print tsql_file
        os.chmod('/tmp/fbrm', stat.S_IROTH)
        print os.popen('ls -al /tmp/*.sql').read()
        with open(tsql_file,'w') as f:
        ...
        if os.path.isfile(spool_file):
            ...
            #print spool
        ttime=datetime.datetime.now().strftime('%Y%m%d%H%M%S')
        fname = '{HOSTNAME}_{DBNAME}_session_lonngops_{TIME}.tmp'
        fname = fname.format(HOSTNAME=self.hostname,DBNAME=sid,TIME=ttime)
        ...
        with open(ora_tmp_file,'w') as fw:
            head_msg=self.getHeadMsg('{HOSTNAME}_{DBNAME}_session_longops.tmp')
            head_msg=head_msg.format(HOSTNAME=self.hostname,DBNAME=sid)
            fw.write(head_msg)
            fw.write('###***HOSTNAME***###\n')
            fw.write(self.hostname+'\n')
            ...
            fw.write('###***ORACLE_SID***###\n')
            fw.write(sid + '\n')
            fw.write('###***ORA_SESSION_LONGOPS***###\n')
            fw.write(spool)
            fw.write('\n')
            fw.write(self.getEndMsg())

        socketClient.SocketSender(FILENAME=ora_tmp_file, DIR='FBRM_ORA_MON', ENDCHECK='NO').main()
        os.remove(ora_tmp_file)
    ...
    def main(self):
        ...
        os.popen('rm -rf /tmp/fbrm/sql_ora/*.sql').read()
        print self.get_data_list()
        ...
        for owner,sid in ora_list:
            ...
            self.get_vsession_longops(sid,owner,env_info)
            self.get_vbackup_files(sid, owner, env_info)
